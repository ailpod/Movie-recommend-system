...
// 删除单个历史记录
const deleteHistoryItem = (movieId) => {
  pendingDeleteMovieId.value = movieId
  confirmDialogTitle.value = '删除观看记录'
  confirmDialogMessage.value = '确定要删除这条观看记录吗？'
...
const handleConfirmDelete = async () => {
  ...
  try {
    await deleteHistoryRecord(pendingDeleteMovieId.value)
    // 从本地数组中移除该记录
    watchHistory.value = watchHistory.value.filter(item => item.movie.id !== pendingDeleteMovieId.value)
